...
my_insert(const void *x, void *base, void *y, int *nelem, int size)
{
    ...
    nbytes = (*nelem) * size - ((uintptr_t)y - (uintptr_t)base);
    assert(nbytes >= 0);
...
int getNextToken(char *token, char *str, char *pos, char *delim) {
    ...
    found = 0;
    while (str[(int)*pos]!='\0') {
        if (strchr(delim,str[(int)*pos])!=NULL) {
            token[len++]='\0';
            while (strchr(delim,str[(int)*pos])!=NULL) {
                *pos = *pos + 1;
            }
            return found;
        }
        else {
            token[len] = str[(int)*pos];
            len++;
            found = 1;
            *pos = *pos + 1;
        }
...
log_base2(int n)
{
  ...
  if (n <= 0 || (n & (n-1)) != 0)
    assert(!"log2() only works for positive power of two values");
