...
static void enum_regular_address(dat_inst_t* d_inst, expr_p expr, int flag,
                int curEq, worklist_p curTSnode, unsigned curAddr) {
    int         i;
    saddr_p      smem;
    ts_p        ts;
    loop_t      *lp;
...
static void analyze_regular_access(dat_inst_t *d_inst, inf_node_t *ib) {
    int         dbg = 0;
    int         i,j,min, tmp;
    int         flag;
    reg_t       tmpReg;
...
void analyze_half_regular_access(dat_inst_t *d_inst, inf_node_t *ib) {
...
int analyze_unpred_access(dat_inst_t *d_inst, inf_node_t* ib) {
    ...
    if (foundRange) {//unpredictable access, but find global var
        ...
        memblk = createSAddr(UNKNOWN_ADDR,insn->addr,0,orgTS);
        addAfterNode(memblk,&blkNode,&(d_inst->addr_set));
        return 0;
    }
    return 0;
...
static int analyze_scalar_access(dat_inst_t *d_inst, inf_node_t* ib) {
    int dbg = 0;
    unsigned addr;
    ...
    blkNode = NULL;
    insn = d_inst->insn;
    memblk = createSAddr(GET_MEM(addr),insn->addr, 1, orgTS);
    addAfterNode(memblk, &blkNode, &(d_inst->addr_set));
    return 0;
}

static void genAccessAddress(dat_inst_t *d_inst, inf_node_t* ib) {
    int dbg = 0;
    de_inst_t      *insn;
...
/*NOTE: bad design*/
static void readExtraCons(char *bin_fname) {
    int dbg = 0;
    FILE *fcons;
    ...
    while (fgets(str,256,fcons)!=0) { 
        pos = 0;
        getNextToken(token, str, (char *)&pos, " ");
        if (dbg) {
...
static void initAll(char *bin_fname) {
    int dbg = 0;
    int i,j;
    loop_t          *loop, *preLoop;
    inf_node_t      *ib;
...
#if 0
static void readSizeCons(char *bin_fname) {
    int dbg = 0;
    FILE *gArray;
    char str[256];
    size_cons_p sCons;
