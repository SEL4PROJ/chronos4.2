...
int mergeTSset(worklist_p dstSet, worklist_p srcSet, int lpId) {
    ...
    while (dstNode && srcNode) {
        ...
        if (cmpLpOrder(dstTS->loop_id,lpId)<0) return flag;
        if (cmpLpOrder(srcTS->loop_id,lpId)<0) return flag;
...
saddr_p createSAddr(int blkAddr, int instAddr, int flag, worklist_p orgTSset) {
    int dbg = 0;
    saddr_p      newBlk;
    worklist_p  blkNode, lpNode;
    ts_p        blkTS,lpTS;

    assert(blkAddr == UNKNOWN_ADDR || blkAddr >= 0);
...
void rearrangeTS(saddr_p smem) {
    worklist_p  curNode, tmpNode, outerNode;
