...
static void tcfg_edge_ctx_term(FILE *fp, tcfg_edge_t *e, int context,
               int upperBound, int bpred, char *prefix, char *suffix);

static void
init_var_grps(void)
{
    num_var_grps = 256;
    var_grps = calloc(num_var_grps, sizeof(*var_grps));
...
add_var(char *var)
{
    if (curr_idx == VAR_GRP_SIZE) {
        if (curr_grp + 1 >= num_var_grps) {
            num_var_grps *= 2;
            var_grps = realloc(var_grps, num_var_grps * sizeof(*var_grps));
        }

        var_grps[++curr_grp] = (char **) calloc(VAR_GRP_SIZE, sizeof(char *));
        curr_idx = 0;
    }
    assert(curr_idx < VAR_GRP_SIZE);
    var_grps[curr_grp][curr_idx] = strdup(var);
    curr_idx++;
}
...
map_bb_bbi(int proc, int bb, int first, int sign, int coeff)
{
    tcfg_nlink_t    *nl;
    tcfg_node_t	    *bbi;
    char	    sign_s[3] = "", tmp[3], coeff_s[16] = "", prefix_str[64];
...
// Disable buggy optimisation when generating ILPs.
#if 0
...
btg_cons(void)
{
	...
	for (e = x->out; e != NULL; e = e->next_out) {
        ...
        /* TODO (bernardb): This was ==, which was a statement with no effect.
         * Hopefully changing this to = only fixes bugs, not uncovers them. */
	    has_root = 0;
...
static void tcfg_estunit_cons_ps(void)
{
	 int edge_id;
	 tcfg_edge_t* e;
...
static void scp_dcache_cost(void) {
    int         i,j;
    tcfg_node_t *bbi;
    cfg_node_t  *bb;
    //dat_inst_t  *d_inst;
...
*/
static void scp_dcache_cons(void) {
    int i,j;
    tcfg_node_t *bbi;
    cfg_node_t  *bb;
    dat_inst_t  *d_inst;
...
tcfg_estunit_cons(void)
{
    ...
    for (i = 0; i < num_tcfg_edges; i++) {
            ...
            int hm = 0;
            int num_hm = num_hit_miss[dst->id];
            char suffix[100];
            tcfg_edge_str(e, BP_NONE);
            for (hm = 0; hm < num_hm; hm++) {
                    sprintf(suffix, ".i%d", hm);
                    fprintf(filp, "d%s.%d", str, hm);
                    int vContext;
                    for (vContext = 0; vContext < num_context; vContext++) {
                            if (flag) {
                                    tcfg_edge_ctx_term(filp, e, vContext, num_context, BP_NONE,
                                                        " - ", suffix);
                            } else {
                                    tcfg_edge_term(filp, e, BP_NONE, " - ", suffix);                             }
                    }
                    fprintf(filp, " = 0\n");
            }
    }
...
static void printContext(int context, int length, char* st) {
        int i;
...
static void tcfg_edge_ctx_term(FILE *fp, tcfg_edge_t *e, int context,
        ...
        char contextStr[l + 10];printContext
        (context, l, contextStr);
        tcfg_edge_str(e, bpred);
        sprintf(term, "%sd%s.v%s%s", prefix, str, contextStr, suffix);
        new_term(fp);
}
static void tcfg_node_ctx_term(FILE* fp, tcfg_node_t* bbi, int context,
                int upperBound, int bpred, char* prefix, char* suffix) {
        if (context >= upperBound) {
                printf("BUGGGGGGG\n");
                exit(0);
        }
        int l = getLength(upperBound - 1);
...
int numberofLoadInst(tcfg_node_t* bbi) {
        int i;
        int ans = 0;
        for (i = 0; i < bbi->bb->num_d_inst; i++) {
                dat_inst_t* dinst = (dat_inst_t*) bbi->bb->d_instlist;
                dinst = dinst + i;
                if (isLoadInst(dinst->insn->op_enum)) {
                        ans++;
                }
        }
        return ans;
}
int intervalLength(int lw, int up, int f) {
        //return up-lw+1;
        if (f == 0) {
                if (lw > 0)
                        return 0;
...
#if 0
int countMiss(worklist_p tsSet, int context, loop_t* lp) {
        int ans = 1;
        worklist_p tsNode = tsSet;
        while (lp != NULL) {
                while (tsNode != NULL) {
                        ts_p ts = (ts_p) tsNode->data;
                        if (cmpLpOrder(lp->id, ts->loop_id) > 0)
                                tsNode = tsNode->next;
                        else
                                break;
                }
                if (tsNode != NULL) {
                        ts_p ts = (ts_p) tsNode->data;
                        if (cmpLpOrder(lp->id, ts->loop_id) == 0) {
                                ans *= intervalLength(ts->lw, ts->up, context & 1);
                        } else {
                                ans *= intervalLength(0, lp->bound - 1, context & 1);
                        }
                }
                context = context >> 1;
                lp = lp->parent;
...
void scp_const_func(void) {
        ...
        for (i = 0; i < num_tcfg_edges; i++) {
                ...
                for (vContext = 0; vContext < num_context; vContext++) {
                        if (flag) {
                                fprintf(filp, " + %d ", extend_cpred_times[i][vContext][0]);
                                tcfg_edge_ctx_term(filp, e, vContext, num_context, BP_NONE,
                                                NO_PREFIX, NO_SUFFIX);
                        } else {
                                fprintf(filp, " + %d ", extend_cpred_times[i][vContext][0]);
...
void scp_context_cons(void) {
        ...
        for (i = 0; i < num_tcfg_edges; i++) {
                ...
                if (num_context > 1) {
                        tcfg_edge_term(filp, e, BP_NONE, NO_PREFIX, NO_SUFFIX);
                        int cId;
                        for (cId = 0; cId < num_context; cId++) {
                                tcfg_edge_ctx_term(filp, e, cId, num_context, BP_NONE, " - ",
                                                NO_SUFFIX);
                        }
                        fprintf(filp, " = 0\n");
                        total_cons++;
        ...
        for (i = 0; i < num_tcfg_nodes; i++) {
                ...
                int num_context = numberofContext[bbi->id];
                if (num_context > 1) {
                        int cId;
                        for (cId = 0; cId < num_context; cId++) {
                                tcfg_node_ctx_term(filp, bbi, cId, num_context, BP_NONE,
                                                NO_PREFIX, NO_SUFFIX);
                                tcfg_edge_t* e;
                                for (e = bbi->out; e != NULL; e = e->next_out) {
                                        tcfg_edge_ctx_term(filp, e, cId, num_context, BP_NONE,
        ...
        fprintf(filp, "\n\\(d)\n");
        extern int *numberOfLoop;
        for (i = 0; i < num_tcfg_loops; i++) {
                loop_t* lp = loops[i];
                tcfg_node_t* lpHead = lp->head;
                int num_context = (numberOfLoop[lpHead->id] - 2);
                if (num_context >= 0) {
                        num_context = 1 << num_context;
                        int cId;
                        for (cId = 0; cId < num_context; cId++) {
                                tcfg_node_ctx_term(filp, lpHead, cId << 1, num_context << 1,
                                                BP_NONE, NO_PREFIX, NO_SUFFIX);
                                tcfg_edge_t* e;
                                for (e = lpHead->in; e != NULL; e = e->next_in) //
                                        if (scp_cmpLpOrder(loop_map[e->src->id]->id, lp->id) > 0) { //TODO: check here
                                                if (num_context > 1)
                                                        tcfg_edge_ctx_term(filp, e, cId, num_context,
                                                                        BP_NONE, " - ", NO_SUFFIX);
                                                else
                                                        tcfg_edge_term(filp, e, BP_NONE, " - ", NO_SUFFIX);
                                        }
        ...
        for (i = 0; i < num_tcfg_loops; i++) {
                ...
                if (num_context >= 0) {
                        ...
                        int cId;
                        for (cId = 0; cId < num_context; cId++) {
                                tcfg_node_ctx_term(filp, lpHead, (cId << 1) | 1,
                                ...
                                for (e = lpHead->in; e != NULL; e = e->next_in)
                                        if (scp_cmpLpOrder(loop_map[e->src->id]->id, lp->id) <= 0) { //TODO: cmplporder <= 0
                                                tcfg_edge_ctx_term(filp, e, cId << 1, num_context << 1,
                                                                BP_NONE, " - ", NO_SUFFIX);
                                                tcfg_edge_ctx_term(filp, e, (cId << 1) | 1,
                                                                num_context << 1, BP_NONE, " - ", NO_SUFFIX);
                                        }
                                fprintf(filp, " = 0\n");
                                total_cons++;
                                ...
                                for (e = lpHead->in; e != NULL; e = e->next_in)
                                        if (scp_cmpLpOrder(loop_map[e->src->id]->id, lp->id) <= 0) { //TODO: cmplporder <= 0
                                                if (lp->bound != 0){
                                                    //if lp->bound is 0, this node is completely unreachable (same as the lp->bound = 1 case, just emit thisNode <= 0.
        ...
        for (i = 0; i < num_tcfg_loops; i++) {
                loop_t* lp = loops[i];
                tcfg_elink_t* link;
                int num_context = numberOfLoop[lp->head->id] - 2;
                if (num_context >= 0) {
                        num_context = 1 << num_context;
                        int cId;
                        for (cId = 0; cId < num_context; cId++) {
                                for (link = lp->exits; link != NULL; link = link->next) {
                                        tcfg_edge_t* e = link->edge;
                                        if (num_context > 1)
                                                tcfg_edge_ctx_term(filp, e, cId, num_context, BP_NONE,
                                                                NO_PREFIX, NO_SUFFIX);
                                        else
        ...
        for (i = 0; i < num_tcfg_nodes; i++) {
                if (numberofContext[i] <= 1)
                        continue;
                tcfg_node_t* bbi = tcfg[i];
                tcfg_edge_t* e;
                int flag = 1;
                for (e = bbi->in; e != NULL; e = e->next_in) {
                        if (loop_map[e->src->id]->id == loop_map[bbi->id]->id) {
                                continue;
                        }

                        loop_t* lp = loop_map[e->src->id];
                        if (lp->parent != NULL && lp->parent->id == loop_map[bbi->id]->id) {
                                continue;
                        }
                        flag = 0;
                        break;
...
//                      for (vContext = 0; vContext < num_context; vContext++) {
//                              int hm = 0;
//                              int max_hm = num_hit_miss[dst->id];
//                              char suffix[100];
//
//                              if (flag) {
//                                      tcfg_edge_ctx_term(filp, e, vContext, num_context, BP_NONE,
...
//                              }
//
//                              for (hm = 0; hm < max_hm; hm++) {
//                                      sprintf(suffix, ".i%d", hm);
//                                      if (flag) {
//                                              tcfg_edge_ctx_term(filp, e, vContext, num_context,
//                                                              BP_NONE, " - ", suffix);
//                                      } else {
//                                              tcfg_edge_term(filp, e, BP_NONE, " - ", suffix);
//                                      }
...
//      }
        /*
         ...
         tcfg_node_t* src = e->src;
         tcfg_node_t* dst = e->dst;
         int numLoad = numberofLoadInst(dst);
         if (numLoad == 0)
         continue;
         int num_context = numberofContext[src->id];
         int flag = (num_context > 1);
         ...
         tcfg_edge_term(filp, e, BP_NONE, NO_PREFIX, NO_SUFFIX);
         tcfg_edge_term(filp, e, BP_NONE, " - ", hitchar);
         tcfg_edge_term(filp, e, BP_NONE, " - ", ncchar);
         }
        ...
        /*fprintf(filp, "\n\\(i)\n");
         ...
         int max_miss = 0;
         int j = 0;
         for (j = 0; j < dst->bb->num_d_inst; j++) {
         dat_inst_t* dinst = (dat_inst_t*) dst->bb->d_instlist;
         dinst = dinst + j;
         max_miss += dinst->max_miss;
         printf("Max miss:%d %d\n", dinst->max_miss,numLoad);
         ...
         tcfg_edge_term(filp, e, BP_NONE, NO_PREFIX, ncchar);
         }
         int max_miss = maxMissEstimate(dst, vContext);
         fprintf(filp, " <= %d\n", max_miss);
         total_cons++;
         }
         }
        ...
        for (i = 0; i < num_tcfg_loops; i++) {
                ...
                if (lp->id == 0) { //the most outer loop
                        tcfg_node_term(filp, head, NO_PREFIX, " <= 1");
                        fprintf(filp, "\n");
                } else {
                        ...
                        tcfg_node_term(filp, head, NO_PREFIX, NO_SUFFIX);
                        tcfg_edge_t* e;
                        for (e = head->in; e != NULL; e = e->next_in) {
                                tcfg_node_t* src = e->src;
                                if (scp_cmpLpOrder(loop_map[src->id]->id, lp->id) > 0) {
                                        fprintf(filp, " - %d ", lp->bound);
                                        tcfg_edge_term(filp, e, BP_NONE, NO_PREFIX, NO_SUFFIX);
                                }
                        }
...
static void
generate_cons_from_imm(void)
{
    // Rewrite the .cons file.
    rewind(fusr);
    ftruncate(fileno(fusr), 0);

    // For every BB, see if it leads to a BB in a different loop.
    ...
    for (pid = 0; pid < prog.num_procs; pid++) {
        ...
        for (bid = 0; bid < p->num_bb; bid++) {
            ...
            // Find the corresponding loop_t.
            int lpid = loop_map[bbi_map[pid][bid]->bbi->id]->id;
            assert(0 <= lpid && lpid < num_tcfg_loops);

            // Incoming edges from outside the loop will form equations.
            int eid;
            for (eid = 0; eid < bb->num_in; eid++) {
                cfg_edge_t *e = bb->in[eid];

                cfg_node_t *src = e->src;

                int from_pid = src->proc->id;
                int from_bid = src->id;
                int from_bbid = bbi_map[from_pid][from_bid]->bbi->id;
                int from_lpid = loop_map[from_bbid]->id;
                if (from_lpid == lpid)
                    continue;
...
void
generate_start_points(void) {
    int i;
    fprintf(filp, "\n\\ === preemption points ===\nb0 ");
    for (i = 0; i < num_start_point; ++i)
        fprintf(filp, "+ b%d ", start_point[i]->id);
    fprintf(filp, "= 1\n");
}
