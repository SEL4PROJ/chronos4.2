...
scan_coexists(void)
{
    ...
    for (inst = eg_len - 2; inst > 0; inst--) { /* TODO: FIXME: This is probably wrong. */
	if (eg_insts[inst]->bp_flag != BP_CPRED) {
...
add_inst(int inst)
{
	...
	if ((stage == STAGE_EX)) {
	    ...
	    node->lat.hi = fu_lat[fu].hi; node->lat.lo = fu_lat[fu].lo;
            ...
            /*
             * This value is 0 if d-cache analysis is enabled.
             * If d-cache analysis is disabled, Chronos assumes single-cycle
             * memory accesses, which does not match reality. We compensate for
             * that here.
             */
            extern int enable_dcache, enable_scp_dcache;
            int mem_access_cycles;
            if (enable_dcache || enable_scp_dcache) {
                mem_access_cycles = 0;
            } else {
...
new_depend(egraph_node_t *src, egraph_node_t *dst, int low, int high, int normal)
{
    ...
    assert(num_eg_edges < MAX_EG_EDGES);
...
data_depends(void)
{
    ...
    /* TODO: br_inst was uninitialised. What should it be initialized to? */
    br_inst = MD_TOTAL_REGS;
...
ready_earlier(int inst1, int inst2)
{
    ...
    if (fu2pfu[(int)v1->fu] != fu2pfu[(int)v2->fu]) {
...
inst_contends(int curr)
{
	...
	if (data_dep[curr][curr-inst] == EG_DEP_NORM)
	    ...
	    normal = EG_NORM_EDGE;
	src = &egraph[inst][STAGE_EX];
	if (fu2pfu[(int)src->fu] == fu2pfu[(int)dst->fu])
