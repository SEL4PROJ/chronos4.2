...
static int getLoopBound(loop_t* loop)
{
  ...
  if(head->exec_count != -1)
  {
	  ...
	  if(parent && parent->head->exec_count != -1) {
		  assert(parent->head->exec_count != 0);
...
{		  
	...
	if(! bbi->bb->out_abs_reg_value) 
	{
		...
		bbi->bb->out_abs_reg_value[28] = (ric_p)malloc(sizeof(ric_s));  
		CHECK_MEM(bbi->bb->out_abs_reg_value[28]);
		bbi->bb->out_abs_reg_value[28]->lower_bound = GLOBAL_START;
		bbi->bb->out_abs_reg_value[28]->upper_bound = GLOBAL_START;
		bbi->bb->out_abs_reg_value[28]->stride = 0;
	}
	...
	for(i = 0; i < MAX_NO_REGISTERS; i++)
	{
            if(bbi->bb->in_abs_reg_value[i]) {
                   free(bbi->bb->out_abs_reg_value[i]); 
	...
	if (bbi->bb->out_abs_mem_value != abs_mem_value) {
	    free(bbi->bb->out_abs_mem_value);
	...
	if(! succ->bb->in_abs_reg_value) {
		*change_flag = 1;  
		...
		for(i = 1; i < MAX_NO_REGISTERS; i++)
		{
			...
			{
                                 free(succ->bb->in_abs_reg_value[i]);
